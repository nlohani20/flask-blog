...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=title, tagline=tagline, slug=slug, date=datetime.now(), content=content,
                             img_file=img_file)
                db.session.add(post)
                # db.session.commit()
            else:
...
def delete(sno):
    post= Posts.query.filter_by(sno=sno)[0]
...
def contact():
    if request.method == 'POST':
        ...
        entry = Contacts(name=name, phone=phone, date=datetime.now(), email=email, msg=message)
        db.session.add(entry)
        db.session.commit()
        mail.send_message('New message from ' + name,
                          sender="111alexleo111@gmail.com",
